...
class RescueFamily(Family):
    ...
    def __init__(self, rescue_data, fam_id, output, merge=True):
        super().__init__(rescue_data, fam_id, output, merge)
        self.filter_domains()
    ...
    def plot_char_rescue(self, label=""):
        ...
        for i, sys in enumerate(self.systems):
            ...
            # Save as SVG
            svg_file = os.path.join(self.output, f"{self.fam_id}/rescue-{sys.sys_id}.svg")
            fig.savefig(svg_file, format='svg', bbox_inches='tight', pad_inches=0.2)
            svgs.append(svg_file)
            plt.close(fig)  # Close the figure to avoid memory issues
        util.combine_svgs(svgs, f"{self.fam_id}/rescue-{self.fam_id}.html", self.output)
    
    def plot(self, options=["char_rescue"]):
        if "char_rescue" in options:
            self.plot_char_rescue()